...
**Motivation:** As an owner of a Bluetooth-enabled "smart" Beurer scales, I am very underwhelmed with their interface for reviewing the measurements done with the scales. To give an example, below to the left is a snapshot from the home screen of their Health Manager app. Frankly, it would be difficult to make this plot *less* informative... While, with multiple clicks, once can get somewhat more useful plots in the app (see the right-hand plot below, note that it cannot be viewed in landscape mode...), the interface is far from intuitive or easy-to-use. Sadly, the web interface is not much better.
